...
export class GroupesComponent implements AfterViewInit,OnInit {
  ...
  instructeur:Array<any>=[];
   ...
   edit(element:any){
     this.service.updateGroupe(Number(this.cookie.get('idPres')),{id:element.id,NomGroupe:element.NomGroupe,activite:{nomActivite:element.activite},instructeur:{id:Number(element.instructeur.split('#',1))?Number(element.instructeur.split('#',1)):0}}).subscribe(
       (res:any)=>{
